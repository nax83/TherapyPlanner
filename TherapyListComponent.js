function createTherapyListComponent(cardId, type, planner) {
    ...
    function buildGridHeader() {
        ...
        const labels = [
            ['#', INDEXCOLWIDTH],
            ['Action', ACTIONCOLWIDTH],
            ['Min Weeks', MINWEEKSCOLWIDTH],
            ['Min Date', MINDATECOLWIDTH],
            ['Date', DATECOLWIDTH],
        ];
    ...
    function buildMinDateCol(item) {
        const col = document.createElement('div');
        col.classList.add(MINDATECOLWIDTH, 'd-flex', 'align-items-center');
        col.textContent = item.status === TherapyPlanner.STATUS_COMPLETED
            ? EM_DASH
            : formatDate(item.earliestSameEyeDate);
        return col;
    }
    ...
    function buildDateInput(item, index) {
        ...
        if (item.status === TherapyPlanner.STATUS_COMPLETED) {
            ...
            dateInput.setAttribute('min', formatDate(item.earliestSameEyeDate));
        }
    ...
    function buildCompletionForm(item, index, actionCol, dateCol) {
        ...
        confirmBtn.addEventListener('click', () => {
            ...
            performPlannerMutation(index, () => planner.setStatus(
                type,
                index,
                TherapyPlanner.STATUS_COMPLETED,
                treatmentDate
            ));
        });
    ...
    function buildPlan() {
        ...
        plan.forEach((item, index) => {
            ...
            const minDateCol = buildMinDateCol(item);
